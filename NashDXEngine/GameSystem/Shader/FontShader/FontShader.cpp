...
bool FontShader::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"../Assets/Shaders/font.vs", L"../Assets/Shaders/font.ps");
	if (!result)
	{
		return false;
	}
	...
	XMMATRIX projectionMatrix, ID3D11ShaderResourceView* texture, XMFLOAT4 pixelColor)
{
	...
	XMMATRIX transposedWorldMatrix;
	...
	XMStoreFloat4x4(&(dataPtr->view), transposedViewMatrix);
	XMStoreFloat4x4(&(dataPtr->projection), transposedProjectionMatrix);
