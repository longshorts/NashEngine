...
bool Terrain::Render(ID3D11DeviceContext* device, Frustum* frustum) {
/*	bool result;
	int i;
	...
	for (i = 0; i<GetCellCount(); i++)
	{
		...
		result = RenderCell(device, i, frustum);
		if (result)
		{
			// Render the cell buffers using the terrain shader.
			result = ShaderManager->RenderTerrainShader(device, GetCellIndexCount(i), worldMatrix, viewMatrix,
...
bool Terrain::GenerateBlendedHeightMap()
{
	...
	for (j = 0; j<m_terrainHeight; j++)
	{
		for (i = 0; i<m_terrainWidth; i++)
		{
			...
			m_heightMap[index].y = (float)height*terrainHeightScale;
	...
	//Smooth down the terrain generated by the image
	SmoothHeightMap();
	SmoothHeightMap();
	//SmoothHeightMap();

	//Blend in the simplex noise to the terrain
	for (j = 0; j<m_terrainHeight; j++)
	{
		for (i = 0; i<m_terrainWidth; i++)
		{
			...
			//Get height (y) from simplex noise
			//m_heightMap[index].y = noise->fractal(4, i, j) + terrainHeightScale*(float)height;
			m_heightMap[index].y += noise->fractal(4, i, j);
		}
	}
...
bool Terrain::SmoothHeightMap() {
	bool result;

	int index;
	float height = 0.0;
	float avgHeight = 0.0;
	HeightMapType* tempMap = new HeightMapType[m_terrainHeight * m_terrainWidth];
	std::vector<float> nHeights;
	//nHeights.push_back(0.0f);
	//std::fill(nHeights.begin(), nHeights.end(), 0);
	...
	for (int j = 0; j<m_terrainHeight; j++)
	{
		for (int i = 0; i<m_terrainWidth; i++)
		{
			...
			//get the average height of all neighbors
			for (int x = -1; x <= 1; x++) {
				for (int y = -1; y <= 1; y++) {
					//Ignore the middle node
					if (x == 0 && y == 0)
						break;
					//Check if the node is within legal bounds of the array
					if (i + x * j + y < 0 || i + x * j + y <= m_terrainHeight * m_terrainWidth) {
						nHeights.push_back(m_heightMap[(m_terrainHeight * j + y) + i + x].y);
					}
				}
			}

			avgHeight = 0.0f;
			for (float f : nHeights) {
				avgHeight += f;
			}
			avgHeight /= nHeights.size();
			nHeights.clear();
